...
void scan() //main program
{
    ...
    std::vector<float> ThatVectorX, ThatVectorY;
    ...
    int counter = 0;
    auto XposSet = tempScWallMapper->GetUniqueXpositions();
    auto YposSet = tempScWallMapper->GetUniqueYpositions();
    std::copy(XposSet.begin(), XposSet.end(), std::back_inserter(ThatVectorX)); //convert set to vector
    std::copy(YposSet.begin(), YposSet.end(), std::back_inserter(ThatVectorY)); //same
